...
public class GoldorHelmet extends AbstractDungeonItem {

    public GoldorHelmet(SimplePlugin plugin) {
        super(plugin, "GoldorHelmet", Material.LEATHER_HELMET);
    }

    @Override
    protected ItemStack generateItem(ItemStack item) {
        SkullMeta meta = (SkullMeta) item.getItemMeta();
        String name = ChatColor.translateAlternateColorCodes('&', "&dGoldor's Helmet");
        meta.setDisplayName(name);

        meta.setLore(ItemLoreGenerator.dungeonItemBaseLoreGenerator(GoldorHelmet.class, item));

        item.setItemMeta(meta);
        CompMetadata.setMetadata(item, "DungeonItem", "GoldorHelmet");
        return item;
    }
    ...
    public LinkedHashMap<PlayerStat, Double> itemStatData() {
        ...
        statData.put(PlayerStat.DEFENSE, 180.0);
        statData.put(PlayerStat.INTELLIGENCE, 30.0);
        return statData;
    }
