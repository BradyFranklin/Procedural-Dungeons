...
public class GoldorChestplate extends AbstractDungeonItem {

    public GoldorChestplate(SimplePlugin plugin) {
        super(plugin, "GoldorChestplate", Material.LEATHER_CHESTPLATE);
    }

    @Override
    protected ItemStack generateItem(ItemStack item) {
        LeatherArmorMeta meta = (LeatherArmorMeta) item.getItemMeta();
        meta.setColor(Color.fromRGB(69, 65, 60));
        String name = ChatColor.translateAlternateColorCodes('&', "&dGoldor's Chestplate");
        meta.setDisplayName(name);

        meta.setLore(ItemLoreGenerator.dungeonItemBaseLoreGenerator(GoldorChestplate.class, item));
        ...
        CompMetadata.setMetadata(item, "DungeonItem", "GoldorChestplate");
        return item;
    }
    ...
    public LinkedHashMap<PlayerStat, Double> itemStatData() {
        ...
        statData.put(PlayerStat.HEALTH, 310.0);
        statData.put(PlayerStat.DEFENSE, 275.0);
