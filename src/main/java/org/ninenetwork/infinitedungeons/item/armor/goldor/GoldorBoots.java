...
public class GoldorBoots extends AbstractDungeonItem {

    public GoldorBoots(SimplePlugin plugin) {
        super(plugin, "GoldorBoots", Material.LEATHER_BOOTS);
    }

    @Override
    protected ItemStack generateItem(ItemStack item) {
        LeatherArmorMeta meta = (LeatherArmorMeta) item.getItemMeta();
        meta.setColor(Color.fromRGB(136, 131, 126));
        String name = ChatColor.translateAlternateColorCodes('&', "&dGoldor's Boots");
        meta.setDisplayName(name);

        meta.setLore(ItemLoreGenerator.dungeonItemBaseLoreGenerator(GoldorBoots.class, item));
        ...
        CompMetadata.setMetadata(item, "DungeonItem", "GoldorBoots");
        return item;
    }
    ...
    public LinkedHashMap<PlayerStat, Double> itemStatData() {
        ...
        statData.put(PlayerStat.HEALTH, 190.0);
        statData.put(PlayerStat.DEFENSE, 165.0);
